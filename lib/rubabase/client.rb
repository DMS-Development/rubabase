...
module Rubabase
  class Client
    ...
    def create_bucket(name, id, file_size_limit = 0, allowed_file_types = [], is_public: true)
      ...
      execute_request(:post, "#{self.configuration.project_url}/bucket/", payload: payload)
    end
    ...
    def update_bucket(name, file_size_limit = 0, allowed_file_types = [], is_public: true)
      ...
      execute_request(:put, "#{self.configuration.project_url}/bucket/#{name}/", payload: payload)
    end
    ...
    #
    def buckets
      execute_request(:get, "#{self.configuration.project_url}/bucket/")
    end
    ...
    def get_bucket(name)
      execute_request(:get, "#{self.configuration.project_url}/bucket/#{name}/")
    end
    ...
    def empty_bucket(name)
      execute_request(:post, "#{self.configuration.project_url}/storage/v1/bucket/#{name}/")
    end
    ...
    def delete_bucket(name)
      execute_request(:delete, "#{self.configuration.project_url}/bucket/#{name}/")
    end
    ...
    # is set to a Bearer token stored in `self.configuration.service_key`.
    ...
    def execute_request(method, url, payload: {}, headers: {})
      default_headers = {
        ...
        'Authorization': "Bearer #{self.configuration.service_key}"
      }
